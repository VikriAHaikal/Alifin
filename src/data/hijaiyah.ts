...
export const getBaseLettersForLevel = (level: number) => {
  ...
  return baseLetters.map(l => {
    ...
    const replaceHarokat = (newHarokat: string, suffix: string, longChar = '') => {
      ...
      else if (baseName.endsWith('a') || baseName.endsWith('o')) baseName = baseName.slice(0, -1);
...
export const getLettersForLevel = (level: number) => {
  ...
  if (volume === 2) {
    ...
    baseLetters.forEach(l => {
      // Short version
      pairs.push({ ...l, hint: 'Dibaca pendek 1 harakat' });
      // Long version
      
      let longChar = l.char.replace('\u064E', '\u064Eا');
      let longArTTS = l.arTTS.replace('\u064E', '\u064Eا');
      ...
      if (l.name.toLowerCase() === "'a" || l.name.toLowerCase() === "a") longName = l.name.toLowerCase() === 'a' ? 'aa' : "'aa";
      
      const capitalizedName = longName.charAt(0).toUpperCase() + longName.slice(1);
      ...
      pairs.push({
        ...
        idTTS: capitalizedName.toLowerCase().replace("'", ' '),
        arTTS: longArTTS,
        hint: 'Dibaca panjang 2 harakat (karena ada Alif)'
      });
    });

    const poolBase = hijaiyahLetters.slice(0, baseLetters[baseLetters.length - 1]?.id || Math.min(level * 4, 28));
    
    const volume2ComboCount = 10;
    for (let i = 0; i < volume2ComboCount; i++) {
       ...
       for (let j = 1; j < comboLength; j++) {
         pickedLetters.push(poolBase[Math.floor(Math.random() * poolBase.length)]);
       }
       
       pickedLetters.sort(() => 0.5 - Math.random());
       
       const transformedLetters = pickedLetters.map(l => {
         // Randomly make it short or long
         ...
         return {
           ...l, char: longChar, name: capName, arTTS: longArTTS, idTTS: capName.toLowerCase().replace("'", ' ')
         };
       });
       ...
       for (let k = 0; k < transformedLetters.length; k++) {
         ...
         if (k === 0) {
           ...
           char += tl.char;
           finalName += (connects ? '' : ' ') + tl.name.toLowerCase();
           idTTS += ' ' + tl.idTTS;
           arTTS += ' ، ' + tl.arTTS;
         }
       }
       
       const formatName = finalName.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ');
       ...
       combos.push({
         ...
         idTTS,
         arTTS,
         hint: 'Latihan rangkaian huruf tersambung dan mad'
       } as any);
    }
    ...
    for (let i = 0; i < otherVolumeComboCount; i++) {
      ...
      const pickedLetters = [];
      pickedLetters.push(baseLetters[Math.floor(Math.random() * baseLetters.length)]); // At least one focal
      ...
      for (let j = 1; j < comboLength; j++) {
        ...
        const randomVolLevel = ((volume - 1) * STAGES_PER_VOLUME) + Math.ceil(rawLetter.id / 4);
        const adaptedArray = getBaseLettersForLevel(randomVolLevel);
        const adapted = adaptedArray.find(a => a.id === rawLetter.id) || rawLetter;
      ...
      for (let k = 0; k < pickedLetters.length; k++) {
        ...
        if (k === 0) {
          ...
          char += pl.char;
          finalName += (connects ? '' : ' ') + pl.name.toLowerCase();
          idTTS += ' ' + pl.idTTS;
          arTTS += ' ، ' + pl.arTTS;
        }
      }
      ...
      combos.push({
        ...
        idTTS,
        arTTS,
        hint: volume === 2 ? 'Latihan huruf tersambung' : 'Rangkaian kalimat: baca berurutan.'
      } as any);
    }
  }
...
export const getQuestionCount = (level: number) => {
  ...
  const isVolume1 = level <= STAGES_PER_VOLUME;
  return isExam ? (isVolume1 ? 15 : 20) : (isVolume1 ? 5 : 10);
};
...
export const generateQuiz = (level: number = 1, count: number = 5): QuizQuestion[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (isSequence) {
      ...
      const decoys = singleLetterPool.sort(() => 0.5 - Math.random()).slice(0, 4).map(l => l.char);
    ...
    if (volume === 2 && isTargetCombo) {
       ...
       if (parts && parts.length === 2) {
          ...
          const allDecoys = [decoy1, decoy2, decoy3, decoy4].filter(d => d !== target.char);
          const customDecoys = allDecoys.sort(() => 0.5 - Math.random()).slice(0, 3);
          
          wrongOptions = customDecoys.map((char, idx) => ({ ...target, id: target.id * 10 + idx, char }));
       } else if (parts && parts.length === 3) {
          const makeShort = (p: string) => p.replace('ا', '');
          const makeLong = (p: string) => p.includes('ا') ? p : p + 'ا';
          
          const decoy1 = makeShort(parts[0]) + makeShort(parts[1]) + makeShort(parts[2]); 
          const decoy2 = makeShort(parts[0]) + makeLong(parts[1]) + makeShort(parts[2]); 
          const decoy3 = makeShort(parts[0]) + makeShort(parts[1]) + makeLong(parts[2]); 
          const decoy4 = makeLong(parts[0]) + makeShort(parts[1]) + makeShort(parts[2]);
    ...
    if (wrongOptions.length < 5) {
      ...
      if (isTargetCombo) {
        ...
        for (let pad = 0; pad < 5 - wrongOptions.length; pad++) {
           wrongOptions.push({ ...target, char: [...parts].sort(() => 0.5 - Math.random()).join(volume === 2 ? '' : ' ') });
        }
      } else {
        const padding = hijaiyahLetters.filter(l => l.id !== target.id && !wrongOptions.find(wo => wo.id === l.id));
        wrongOptions.push(...padding.sort(() => 0.5 - Math.random()).slice(0, 5 - wrongOptions.length));
      }
    }
